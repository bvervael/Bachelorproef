...
% BVV: Ontbreekt er ook niet een overzicht van de Elastic Stack als geheel? Een afbeelding van de architectuur of de flow van informatie doorheen het systeem zou ook een meerwaarde zijn.

\chapter{Logstash}
\label{ch:logstash}

% BVV: deze sectiehoofding is overbodig. De eerste paragraaf van een hoofdstuk is sowieso een inleiding. In een inleidende paragraff van een hoofstuk licht je ook toe wat er in het hoofstuk staat, waarom je het bespreekt en hoe het past binnen de rode draad van je bachelorproef, meer bepaald hoe het gerelateerd is aan het vorige en volgende hoofstuk.

\section{Introductie}
\label{sec:logstash-introductie}

% BVV: deze paragraaf lijkt nog niet af, maar een samenraapsel van zaken die je zeker wilt vermelden. Ik heb het dan ook niet meer voorbij de eerste zinnen in detail doorgenomen.

Logstash is een open source data verzamel programma. Het werkt real-time en zal dus reageren van zodra er nieuwe data beschikbaar is.  Logstash beschik over de mogelijkheid de data te normalizeren. 
% BVV: "data verzamel programma" = letterlijke vertaling uit het Engels? Gebruik een correct Nederlands woord. En het gaat toch over iets specifiekers dan "data" in het algemeen?
% BVV: Wat betekent "normalizeren"?
...
\end{lstlisting}
% BVV: De grijze achtergrond en de kleuren zullen de code onleesbaar maken als je dit afdrukt in zwart-wit. Je maakt de achtergrond best wit. 
...
% BVV: let op: Engelse termen als er Nederlandse alternatieven bestaan
...
Hier gebeuren alle bewerkingen en zal bepaald worden wat in de JSON gaat en wat niet. In de filter is het de bedoeling dat de file zoveel mogelijk genormaliseerd wordt. Een log lijn wordt eerst volledig ingelezen in een field  genaamd “message”. Van daaruit is het de bedoeling dat vast voorkomende stukken tekst eruit gehaald worden en in een eigen veld geplaatst worden, indien deze later gebruikt willen worden.  Om deze speciefieke velden te maken zijn enkele mogelijkheden deze zullen besproken worden in subsec:patroon} en \ref{subsec:zoeken}. Werken met de tijd is niet altijd even simpel en zal daarom afzonderlijk besproken worden in \ref{subsec:timestamp}. Er zal afgesloten worden in \ref{subsec:overige} met nog enkele nuttige commando’s. 
Hou er rekening mee dat elasticsearch zal zoeken aan de hand van de fields die hier gemaakt worden. De juiste field keuzes kunnen dus een grote invloed hebben op de zoeksnelheid.
...
% BVV: Welke andere zijn er en waarom enkel deze drie bespreken?
